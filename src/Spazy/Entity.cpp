...
void Entity::draw(KingPin::SpriteBatch &spriteBatch) {
    if (_entityStatus != DESTROYED) {
        ...
        glm::vec4 mirrorDownRect(_position.x, _position.y - _worldHeight,
                                 _width, _height);
        ...
        glm::vec4 mirrorLeftRect(_position.x - _worldWidth, _position.y, _width,
                                 _height);
        glm::vec4 mirrorRightRect(_position.x + _worldWidth, _position.y,
                                  _width, _height);
        ...
        spriteBatch.draw(mirrorDownRect, _angle, uvRect, _textureID, 1.0f,
                         color);
        spriteBatch.draw(mirrorUpRect, _angle, uvRect, _textureID, 1.0f, color);
        spriteBatch.draw(mirrorLeftRect, _angle, uvRect, _textureID, 1.0f,
                         color);
        spriteBatch.draw(mirrorRightRect, _angle, uvRect, _textureID, 1.0f,
                         color);
    }

    drawEffects(spriteBatch);
}
void Entity::setLife(float life) { _life = life; }
...
float Entity::getSpeed() const { return glm::length(_velocity); }

float Entity::getAngle() const {
    const static float range = 2. * M_PI;
    return fmod(std::abs(_angle), range);
}

float Entity::getLife() const { return _life; }
void Entity::checkPosition() {
    if (_position.y > _worldHeight / 2) _position.y = -_worldHeight / 2;
    if (_position.y < -_worldHeight / 2) _position.y = _worldHeight / 2;
    if (_position.x > _worldWidth / 2) _position.x = -_worldWidth / 2;
    if (_position.x < -_worldWidth / 2) _position.x = _worldWidth / 2;
}
